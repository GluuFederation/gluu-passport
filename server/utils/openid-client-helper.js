...
const
	...
	JWKS = "jwks",
	AZP = "additionalAuthorizedParties"
...
function getClient(options) {
	let issuer_meta = populate(options, issuerProperties),
		metadata = populate(options, clientProperties),
		jwks, azp
	...
	//console.log(JSON.stringify(issuer_meta))
	//console.log(JSON.stringify(metadata))
	let issuer = new Issuer(issuer_meta)
...
function getResidualObject(options, props) {
	let keys = R.keys(options),
		indexOf = (R.flip(R.indexOf))(props),
		residual = {}
...
function makeOptions(options) {
	...
	let client = getClient(options),
		params = populate(options, authzRequestParams),
		allProps = R.concat(R.concat(issuerProperties, clientProperties), authzRequestParams),
		opts = getResidualObject(options, allProps)
