...
class PersonAuthentication(PersonAuthenticationType):
    ...
    def init(self, configurationAttributes):
        ...
        if not configurationAttributes.containsKey("key_store_file"):
            print "Passport-saml: Initialization. The property key_store_file is mandatory"
            return False

        if not configurationAttributes.containsKey("key_store_password"):
            print "Passport-saml: Initialization. The property key_store_password is mandatory"
            return False
    ...
    def destroy(self, configurationAttributes):
        print "Passport-saml: Basic. Destroy method call"
        print "Passport-saml: Basic. Destroyed successfully"
        return True
    ...
    def getUserValueFromJwt(self, user_profile, remote_attr):
        try:
            ...
            print("Passport-saml: Exception inside getUserValueFromJwt " + str(err))
            return None
    ...
    def authenticate(self, configurationAttributes, requestParameters, step):
        ...
        if StringHelper.isNotEmpty(UserId):
            ...
            if StringHelper.isEmpty(jwt_param):
                print "Passport-saml: Authenticate for step 1. JWT token is missing"
                return False
            ...
            # Validate signature
            print "Passport-saml: Authenticate for step 1. Checking JWT token signature: '%s'" % jwt
            ...
            print "Passport-saml: Authenticate for step 1. JWT signature validation result: '%s'" % valid
            if not valid:
                print "Passport-saml: Authenticate for step 1. JWT signature validation failed"
                return False
            ...
            if StringHelper.isEmpty(user_profile_json):
                print "Passport-saml: Authenticate for step 1. User profile is missing in JWT token"
                return False
            
            user_profile = json.loads(user_profile_json)
                
            # Store user profile in session
            print "Passport-saml: Authenticate for step 1. User profile: '%s'" % user_profile_json
    ...
    def prepareForStep(self, configurationAttributes, requestParameters, step):
        ...
        if step == 1:
            print "Passport-saml: Prepare for step 1"
            identity = CdiUtil.bean(Identity)
